...
4. 内容池深度：当前已达到 40 武器 / 48 遗物，并具备全条目 SVG 图标、Field/Mine/Sentry 部署、Homing/Chain 路线、七类 authored 武器音效、跨奖励房/普通宝箱的 Rare+ 暴露保底、完整局稀有度模拟和可随 run seed 复现的奖励随机流；后续优先补武器奖励节奏、更多部署行为、武器专属命中反馈和特殊交互。
