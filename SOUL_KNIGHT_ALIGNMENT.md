...
1. 武器池：扩充近战、激光、爆炸、轨迹类武器，并让能耗和弹幕强度形成明显取舍。
2. 地牢层级：把当前房间图扩展为 3 个主题层，每层末尾固定 Boss。
3. 特殊房间：补充武器宝箱房、事件房、治疗/补给房、挑战房。
4. 战斗外大厅：加入角色预览、统计入口、角色解锁和轻量永久成长。
5. 角色深度：让角色技能与遗物、天赋、雕像/祝福类系统产生可读的组合变化。
