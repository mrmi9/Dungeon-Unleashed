...
### 遗物奖励来源保底与稀有度下限第一版
- `RelicDropTableData` 新增硬稀有度下限与共享保底配置；奖励房和普通宝箱共享 `relic_reward` 计数，连续 3 组无 Rare+ 后下一组至少暴露一个 Rare+。
- Premium/Boss Chest 固定 Rare+ 下限；Shop 继续使用独立加权曲线，不推进或消耗普通奖励保底。
...
### 中后程 HUD、小地图与战斗文字性能修复
- 完整 42 房间性能测试定位到两个主尖峰：每帧角色被动 HUD 重复解析约 54.7 ms，以及每次房间状态变化重建整张小地图约 1166.9 ms。
- 被动状态改为静态图标提示缓存和 10 Hz 动态计时；小地图改为拓扑变化才重建、普通推进只更新状态变化标记，headless 单次分别降至约 0.033 ms 和 0.119 ms。
- 战斗文字改为 24 个预热、48 个上限的循环池；全部未进入房间关闭逐帧重叠轮询，改用入口信号和一次初始检查。
- 新增 `RuntimePerformanceSmokeTest` 并通过 Windows/OpenGL、地牢生成、战斗反馈、角色、武器、内容管线、菜单、完整房间流与完整 Run 回归；后两项执行时间分别从既有约 247s/265.6s 降至 65.6s/68.8s。
...
### 简体中文运行时与后续开发规范
- 正式游戏默认使用简体中文，完成角色、武器、遗物、天赋、祝福、雕像、敌人、三层 Biome、房间、奖励、商店、训练、设置、HUD、大厅与结算的统一中文显示。
- 保留稳定英文内容 ID、资源路径、存档键和代码符号，中文仅作用于玩家显示层；图标工具提示不再暴露内部键值。
- 新增中文内容覆盖、隐藏界面残留、真实 Main/大厅摘要和扫描性能专项测试；增量扫描使用节点缓存，双 UI 场景稳定耗时约 `0.597 ms/次`，不重新引入周期性卡顿。
- 新增 `CHINESE_LOCALIZATION_GUIDE.md`，规定后续新增玩家可见内容优先使用中文，并将 Windows 中文字体、换行和低分辨率布局检查纳入验收。

## 第二百七十批已落地靠拢改动

### 图标优先的紧凑战斗 HUD
- 左侧战斗信息从 `268×352` 收紧为 `220×236`，生命/护甲/能量与金币/敌人数分别合并为横向信息行，常驻面积减少约 45%。
- 当前武器区移除重复武器名、重复弹药和常驻元数据，三武器槽只显示可辨识图标、激活边框与状态颜色；完整名称、稀有度、类型、射程、能耗和弹药保留在悬停提示中。
- 当前武器名称、弹药数字、弹匣段、装填状态及能量不足反馈继续常驻，避免精简布局削弱战斗判断。
- 新增紧凑尺寸、重复行隐藏、武器图标可见和长标签隐藏断言，并通过武器、中文化与运行时性能回归。

## 第二百七十一批已落地靠拢改动

### 图标化中文结算战报
- 结算页新增胜负图标和房间、击杀、用时、首领四项图标指标；概览、构筑、生存、战斗、战利品、记录六个分区均以图标建立扫描入口。
- 三个结算操作合并为横向动作行，详细统计继续放在可滚动区域，并保留紧凑/详情模式及全部历史、永久货币、熟练度数据。
- 中文运行时不再依赖逐词替换完整英文战报，而是直接从结构化 Run summary 生成中文路线、位置、首领、特殊房间、构筑和记录文本，消除中英混排。
- 小地图种子悬停提示不再暴露内部布局 ID；调试详情保留在 `F3` 专用面板。
- 所有流程面板统一隐藏战斗 HUD 与小地图；结算额外关闭旧胜负横幅和 Boss 血条，消除战报后方的视觉重叠。
- 新增结果图标、顶部指标、单行动作和完整中文战报断言，并通过结算、中文化、布局、菜单、完整三层 Run 与运行时性能回归。

## 第二百七十二批已落地靠拢改动

### 完整局遗物稀有度模拟与传说遗物
- 新增 `RelicRewardSimulation`，按真实三层路线模拟 3 次奖励房三选一、4 个优质宝箱、3 个首领宝箱和 3 次商店遗物展示；随机选择和最高稀有度选择分别维护真实持有状态。
- 新增折光冠冕、永动机芯和黑星继电器三件原创传说遗物，分别提供额外 2 枚弹丸、7 点能量上限和额外 2 个连锁目标；三件遗物均接入五类掉落表、中文显示和专属 SVG 图标。
- 1500 局固定种子模拟中，随机选择平均每局获得 Rare+ `7.88` 件、Epic+ `0.97` 件，至少获得一件传说遗物的完整局占比为 `5.7%`；最高稀有度选择下传说局占比为 `6.8%`。
- `RewardDistributionSmokeTest` 锁定来源数量、同种子复现、异种子变化、共享保底、硬稀有度下限、重复选项、真实获取和带容差的分布区间。
- 大厅熟练度满级状态补充“已满级”翻译，维持正式界面全中文约束。

## 第二百七十三批已落地靠拢改动

### 完整局武器奖励节奏与来源表
- 新增 `WeaponDropTableData` 和 `WeaponRewardPicker`，军械库、商店、首领箱、诅咒事件分别使用独立生产来源表；先按来源权重选择稀有度，再在该档位内按武器 `drop_weight` 选择，消除条目数量对稀有度概率的隐性偏置。
- 军械库和商店覆盖全部 39 把非初始武器，首领箱覆盖全部 32 把 Rare+ 武器并强制 Rare+，诅咒事件覆盖全部 15 把 Epic+ 武器并强制 Epic+。
- 删除 `RewardChest`、`ShopInventory`、`EventShrine` 和首领箱场景中重复维护的武器数组，生产候选池只由四个 `.tres` 来源表定义；脚本仍保留可注入 fallback 池供专项测试使用。
- 新增 `WeaponRewardSimulation` 和 `WeaponRewardPacingSmokeTest`，按真实三层路线模拟 3 个军械库、3 个首领箱、3 个商店展示及每层 `1/6` 概率的诅咒事件，并分别统计保证奖励和武器优先购买策略。
- 3000 局固定种子结果中，军械库 Rare+ `47.9%`、首领箱 Epic+ `26.8%`、商店 Epic+ `19.4%`、诅咒事件传说 `13.32%`；保证奖励平均每局 `1.70` 把 Epic+，传说武器局占比 `19.7%`。
- 保证奖励路线平均覆盖 `4.31` 类武器和 `3.06` 种开火模式；近战、环形、蓄力、部署形态的完整局出现率分别为 `80.8%`、`48.5%`、`36.3%`、`41.2%`。

## 第二百七十四批已落地靠拢改动

### 战斗 HUD 二次精简与当前层小地图
- 左上常驻战斗区从上一版 `220×236` 继续压缩到不超过 `190×190`，实际 720p 画面约为 `182×184`，相比上一版再减少约三分之一占用面积。
- 生命、护甲和能量改为 14px 语义图标加纯数值；低血与能量不足使用感叹号、颜色和既有脉冲，不再追加长句。
- 角色被动常驻区只保留角色图标，完整被动说明放入悬停提示；祝福/雕像规则行默认隐藏，仅在触发后短暂显示图标和名称。
- 遗物常驻文本改为数量摘要，完整遗物列表保留在悬停提示；重复房间状态行移除，由中心房间提示和右上小地图承担。
- 小地图仍维护全部房间 marker 与三层结构，但界面只显示当前 Biome，marker 从 `24×22` 缩到 `18×18`，避免三层路线横向溢出覆盖顶部战斗画面。
- `UILayoutSmokeTest` 将常驻战斗区上限收紧为 `190×190` 并验证三项状态图标、隐藏长文本和临时规则行；`DungeonGenerationSmokeTest` 新增仅一个 Biome 层可见的断言。
...
4. 内容池深度：当前已达到 40 武器 / 48 遗物，并具备全条目 SVG 图标、Field/Mine/Sentry 部署、Homing/Chain 路线、七类 authored 武器音效、武器/遗物来源表、跨奖励房/普通宝箱的 Rare+ 暴露保底、完整局稀有度与形态模拟和可随 run seed 复现的奖励随机流；后续优先补更多部署行为、武器专属命中反馈和特殊交互。
