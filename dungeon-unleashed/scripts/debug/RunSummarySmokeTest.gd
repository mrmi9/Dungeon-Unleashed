...
func _run() -> void:
	...
	_expect(str(hud.call("get_result_subtitle_text")).contains(str(summary.get("dungeon_seed", 0))), "Result headline should expose the active seed")
	_expect(str(hud.call("get_result_quick_stat_text", "rooms")).contains("房间 1"), "Result quick stats should show cleared rooms in Chinese")
	_expect(str(hud.call("get_result_quick_stat_text", "kills")).contains("击杀 1"), "Result quick stats should show kills in Chinese")
	_expect(str(hud.call("get_result_quick_stat_text", "time")).contains("用时"), "Result quick stats should show elapsed time in Chinese")
	_expect(str(hud.call("get_result_quick_stat_text", "bosses")).contains("首领"), "Result quick stats should show defeated bosses in Chinese")
	_expect(str(hud.call("get_result_outcome_icon_texture_path")).ends_with("room_boss.svg"), "Victory result should use the boss-room outcome icon")
	_expect(str(hud.call("get_result_section_icon_texture_path", "Overview")).ends_with("default_room.svg"), "Result overview should use a room summary icon")
	_expect(str(hud.call("get_result_section_icon_texture_path", "Build")).ends_with("default_weapon.svg"), "Result build should use a weapon summary icon")
	_expect(int(hud.call("get_result_action_count")) == 3, "Result actions should share one three-button row")
	_expect(not bool(hud.call("is_gameplay_stats_visible_for_test")), "Result screen should hide combat stats behind the report")
	_expect(not bool(hud.call("is_minimap_visible_for_test")), "Result screen should hide the minimap behind the report")
	_expect(not bool(hud.call("is_completion_visible")), "Result screen should replace the legacy completion banner")
